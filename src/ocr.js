...
function buildPrompt() {
  ...
  return `
    ...
    - account_last4: เลขท้ายของบัญชีต้นทาง (ผู้โอน/ผู้ถอน) ตามจำนวนหลักที่ปรากฏจริง (2-6 หลัก) เก็บเฉพาะตัวเลข ถ้าไม่พบให้เป็น "null"
      *** ยกเว้นสลิปฝาก/เงินเข้า/รับโอน (tx_type = Deposit, ฝากเงิน, เงินเข้า, รับเงิน) ที่ไม่มีบัญชีต้นทาง → ให้ account_last4 = เลขบัญชี "ที่เงินเข้า/ปลายทาง/ผู้รับ" (TO A/C, ผู้รับ) แทน เพราะสลิปฝากให้ยึดบัญชีที่รับเงินเป็นหลัก ***
    - amount: ยอดเงิน (ตัวเลข ไม่มีลูกน้ำ)
      *** สลิปฝาก/เงินเข้า: ถ้ามีบรรทัด "NET DEPOSIT AMOUNT" / "ยอดสุทธิ" / "ยอดเข้าบัญชีจริง" (ยอดหลังหักค่าธรรมเนียมที่เข้าบัญชีจริง) ให้ใช้ค่านั้นเป็น amount ไม่ใช่ยอดฝากรวม (AMOUNT/เงินสดที่ใส่) — เช่น AMOUNT 138,000 / FEE 148 / NET DEPOSIT AMOUNT 137,852 → amount = 137852 ***
    ...
    - สลิป ATM กระดาษมักพิมพ์วันที่แบบ DD/MM/YY (วัน/เดือน/ปี) และตัวเลขอาจจาง — อ่านหลัก "เดือน" ให้ละเอียด
    - เฉพาะกรณีตัวเลขจาง/กำกวมเท่านั้น ให้เลือกค่าที่ทำให้วันที่ "ไม่เกินวันนี้" และใกล้วันนี้ที่สุด ห้ามตอบวันที่ในอนาคต
    - ถ้าตัวเลขชัดเจนอยู่แล้ว ให้ตอบตามที่เห็นจริง อย่าดัดให้เป็นวันนี้
...
const ACCOUNT_HINT = `
    ...
    *** สำคัญมาก ***
    - account_last4 = บัญชี "ต้นทาง/ผู้โอน" (FROM A/C, ผู้โอน, From) เท่านั้น
      ...
      *** ยกเว้นสลิปฝาก/เงินเข้า (Deposit, ฝากเงิน, เงินเข้า, รับโอน) ที่ไม่มีบัญชีต้นทาง → ใช้บัญชี "ที่เงินเข้า/ผู้รับ" (TO A/C) เป็น account_last4 ***
    - amount/fee: คงทศนิยมให้ครบเป๊ะตามสลิป (เช่น 24,464.26 ต้องเป็น 24464.26 ห้ามตัดเป็น 246426 หรือ 24464)
    - สลิปฝาก/เงินเข้า: ใช้ "NET DEPOSIT AMOUNT/ยอดสุทธิ" (ยอดเข้าบัญชีจริงหลังหักค่าธรรมเนียม) เป็น amount ถ้ามี ไม่ใช่ยอดฝากรวม
`;
